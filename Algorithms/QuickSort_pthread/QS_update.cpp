...
pthread_t final_threads[THREAD_NUM];
struct arg_struct final_arguments[THREAD_NUM];
...
void * finalQuickSortStarter(void * arg) {
	...
	int * tab = final_arguments[id].table_;
	int left_border = final_arguments[id].left_;
	int right_border = final_arguments[id].right_;
...
void threadQuickSort(int tab[], int temp_tab[], int left_border, int right_border, int proc, int dir) { // MAIN QS FUNCTION
	...
	if (proc == 1) {
		final_arguments[thread_start].table_ = tab;
		final_arguments[thread_start].left_ = left_border;
		final_arguments[thread_start].right_ = right_border;

		pthread_create(&final_threads[thread_start], NULL, finalQuickSortStarter, (void *)thread_start);
		pthread_detach(final_threads[thread_start]);
		thread_start++;
		/*
		printf("FINAL QS\n");
		for (int k = 0; k < SIZE; k++) {
			if (left_border == k) printf("| ");
			printf("%d ", tab[k]);
			if (right_border == k) printf("| ");
		}
		printf("\n");
		*/
	} else {
		...
		if (proc_left > 0) threadQuickSort(tab, temp_tab, left_border, splitter - 1, proc_left, 1);
		if (proc_right > 0) threadQuickSort(tab, temp_tab, splitter, right_border, proc_right, 1);
	}
	if (dir == 0) {
